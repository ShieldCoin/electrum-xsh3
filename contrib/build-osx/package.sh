...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-xsh-macos
	cd /tmp/electrum-xsh-macos
...
rm -rf /tmp/electrum-xsh-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-xsh-macos/image/
cp -r $1 /tmp/electrum-xsh-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-XSH_uncompressed.dmg \
    /tmp/electrum-xsh-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-XSH_uncompressed.dmg electrum-xsh-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-XSH_uncompressed.dmg

echo "Done."
md5sum electrum-xsh-$VERSION.dmg
