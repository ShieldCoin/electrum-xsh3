...
version=`python3 -c "import electrum_xsh; print(electrum_xsh.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://electrum-xsh.org/download/$f"
    wget -q https://electrum-xsh.org/download/$f -O signed/$f
