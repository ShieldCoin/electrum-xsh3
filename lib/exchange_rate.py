...
class Bit2C(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bit2c.co.il', '/Exchanges/XSHNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("XSH", ""), Decimal(json[r]['last']))
    ...
    def request_history(self, ccy):
        history = self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/XSH%s?period=alltime&format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        rates = [(r, json['XSH'][r]) for r in json['XSH']
                 if json['XSH'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    def request_history(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=XSH")[ccy +'_XSH']

class Bitfinex(ExchangeBase):

    def get_rates(self, ccy):
        json = self.get_json('api.bitfinex.com', '/v1/pubticker/xshusd')
...
class Bitso(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.bitso.com', '/v3/ticker/?book=xsh_mxn')
...
class BitStamp(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bitstamp.net', '/api/v2/ticker/xshusd/')
...
class Coinbase(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=XSH')
...
class CoinSpot(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['xsh']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        xsh_prices = json['prices']['XSH']
        return dict([(r, Decimal(xsh_prices[r])) for r in xsh_prices])
...
class HitBTC(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ccys = ['EUR', 'USD']
        json = self.get_json('api.hitbtc.com', '/api/1/public/XSH%s/ticker' % ccy)
...
class Kraken(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XXSHZ')]
        json = self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XXSHZ'+ccy]['c'][0])
        return result
    ...
    def request_history(self, ccy):
        query = '/0/public/OHLC?pair=XSH%s&interval=1440' % ccy
        json = self.get_json('api.kraken.com', query)
        history = json['result']['XXSHZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=xsh_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    def get_rates(self,ccy):
        json = self.get_json('www.mercadobitcoin.net', '/api/xsh/ticker/')
...
class TheRockTrading(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.therocktrading.com', 
                             '/v1/funds/XSHEUR/ticker')
        return {'EUR': Decimal(json['last'])}

class QuadrigaCX(ExchangeBase):

    def get_rates(self,ccy):
        json = self.get_json('api.quadrigacx.com', '/v2/ticker?book=xsh_cad')
...
class WEX(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json_eur = self.get_json('wex.nz', '/api/3/ticker/xsh_eur')
        json_rub = self.get_json('wex.nz', '/api/3/ticker/xsh_rur')
        json_usd = self.get_json('wex.nz', '/api/3/ticker/xsh_usd')
        return {'EUR': Decimal(json_eur['xsh_eur']['last']),
                'RUB': Decimal(json_rub['xsh_rur']['last']),
                'USD': Decimal(json_usd['xsh_usd']['last'])}
